...
class DatasetCatalog:
    ...
    def get(name):
        ...
        elif 'my_custom_dataset' in name:
            my_custom_dataset = DatasetCatalog.DATA_DIR
            attrs = DatasetCatalog.DATASETS[name]
            args = dict(
                    data_dir = os.path.join(my_custom_dataset, attrs['data_dir']),
                    ann_file = os.path.join(my_custom_dataset, attrs['ann_file']),
            )
            return dict(factory="MyDataset", args=args)
        elif 'my_custom_test_dataset' in name:
            my_custom_test_dataset = DatasetCatalog.DATA_DIR
            attrs = DatasetCatalog.DATASETS[name]
            args = dict(
                    data_dir = os.path.join(my_custom_test_dataset, attrs['data_dir']),
                    ann_file = os.path.join(my_custom_test_dataset, attrs['ann_file']),
            )
            return dict(factory='MyDataset', args=args)
